...
pub fn render_search_rss(query: &str, tweets: &Tweets, config: &Config) -> String {
   ...
   let description = format!("Twitter feed for: Search \"{query}\". Generated by {url_prefix}");
   rss_channel(
...
/// Helper to get a signed proxied video URL path (not full URL).
fn get_video_url(url: &str, config: &Config) -> String {
   formatters::get_vid_url(url, &config.config.hmac_key, config.config.base64_media)
}
